-- Workspaces Schema for Tyform (Fixed RLS)
-- Run this in Supabase SQL Editor

-- Drop existing policies if they exist (to fix infinite recursion)
...
-- Drop existing workspace policies if they exist
...
-- Workspace Members: Use workspace owner check instead of self-referencing
-- This avoids infinite recursion by checking the workspaces table directly
...
CREATE POLICY "Owners and admins can add members"
  ...
  WITH CHECK (
    ...
    OR 
    -- User adding themselves (accepting invite)
    auth.uid() = user_id
  );
